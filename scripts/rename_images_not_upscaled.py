...
# Start counter after the highest existing image-N number in the images dir
images_dir = os.path.join(site_dir, "images")
existing = [f for f in os.listdir(images_dir) if re.match(r'image-(\d+)\.', f)]
highest = max((int(re.search(r'image-(\d+)', f).group(1)) for f in existing), default=0)
counter = highest + 1

print(f"Found {len(unique_srcs)} unique images in the webpage. Starting counter at {counter}.")
...
for old_src in unique_srcs:
    ...
    # Skip UniversalUpscaler images and already-renamed image-N files
    if old_filename.startswith('UniversalUpscaler') or re.match(r'image-\d+\.', old_filename):
        continue
        
    # (old_filename already set above)
